...
export function setInverse<T extends Model>(record: Model, attribute: string, handlingClass: ModelClass<T>, otherRecordId: string, options: BelongsToHandlerOptions | HasManyHandlerOptions, link: boolean): void {
    ...
    const otherRecord = record.store.peekRecord(handlingClass, otherRecordId);
