...
export function belongsTo<T extends Model>(recordClass: ModelClass<T>, options: BelongsToHandlerOptions = {}): AttributeHandlerAndType<T> {
    return {
        ...
        handler: (attribute: string): AttributeHandler => {
            return {
                ...
                set: <U, V extends Model | Serializer>(record: Model, value: U, handlingClass: ModelClass<V>): U => {
                    if (record.isDeleted) {
